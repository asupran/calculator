...
function displayTwoCountryComparison() {
    ...
    if (domElements.winnerCard && domElements.winnerText) {
        if (countryAProfit > countryBProfit) {
            ...
            const percentageAdvantage = (advantage / countryBProfit) * 100;
            domElements.winnerText.textContent = `${countryNames[selectedCountryA]} is better by $${advantage.toFixed(2)} per unit (${percentageAdvantage.toFixed(1)}% more profit)`;
            ...
            const percentageAdvantage = (advantage / countryAProfit) * 100;
            domElements.winnerText.textContent = `${countryNames[selectedCountryB]} is better by $${advantage.toFixed(2)} per unit (${percentageAdvantage.toFixed(1)}% more profit)`;
    ...
    // Show cost impact vs China automatically
    updateCostImpactDisplay();
}

function updateCostImpactDisplay() {
    if (!currentProduct || !selectedCountryA || !selectedCountryB) {
        return;
    }
    
    // Calculate China cost for comparison baseline
    const chinaCost = calculateCountryCost('china');
    
    // Determine which country to compare vs China (the non-China country)
    let comparisonCountry, comparisonCost;
    
    if (selectedCountryA === 'china') {
        comparisonCountry = selectedCountryB;
        comparisonCost = countryBCost;
    } else if (selectedCountryB === 'china') {
        comparisonCountry = selectedCountryA;
        comparisonCost = countryACost;
    } else {
        // If neither is China, compare the selected winner vs China
        comparisonCountry = selectedCountryA; // Default to Country A
        comparisonCost = countryACost;
    }
    
    // Calculate cost difference vs China (can be positive or negative)
    const costDifference = comparisonCost.totalCost - chinaCost.totalCost;
    const costDifferencePercent = (costDifference / chinaCost.totalCost) * 100;
    ...
    if (costIncreaseAmountElement) {
        if (costDifference >= 0) {
            costIncreaseAmountElement.textContent = '+$' + costDifference.toFixed(2);
            costIncreaseAmountElement.style.color = '#e74c3c';
        } else {
            costIncreaseAmountElement.textContent = '-$' + Math.abs(costDifference).toFixed(2);
            costIncreaseAmountElement.style.color = '#27ae60';
        }
    }
    if (costIncreasePercentElement) {
        costIncreasePercentElement.textContent = (costDifference >= 0 ? '+' : '') + costDifferencePercent.toFixed(1) + '%';
        costIncreasePercentElement.style.color = costDifference >= 0 ? '#e74c3c' : '#27ae60';
    }
    if (costImpactSummaryElement) {
        costImpactSummaryElement.style.display = 'block';
    }
}
...
function showPricingStrategy(strategy) {
    ...
    // Determine which country to use for pricing strategy
    let targetCountry, targetCost;
    
    if (selectedCountryA === 'china') {
        // If Country A is China, use Country B for pricing strategy
        targetCountry = selectedCountryB;
        targetCost = countryBCost;
    } else if (selectedCountryB === 'china') {
        // If Country B is China, use Country A for pricing strategy
        targetCountry = selectedCountryA;
        targetCost = countryACost;
    } else {
        // If neither is China, use the more expensive country
        targetCountry = countryACost.totalCost >= countryBCost.totalCost ? selectedCountryA : selectedCountryB;
        targetCost = countryACost.totalCost >= countryBCost.totalCost ? countryACost : countryBCost;
    }
    
    const newCOGS = targetCost.totalCost;
    
    // Calculate China baseline for comparison
    const chinaCost = calculateCountryCost('china');
    const chinaTotalCost = chinaCost.totalCost;
